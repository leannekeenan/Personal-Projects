...
router.post('/', async (req, res) => {
  try {
    ...
    if (currentHour >= 14 || currentHour < 9) { 
        return res.status(403).json({ message: "The tavern is closed for the day! Preordering resets next week at 9AM." });
    }
...
cron.schedule('0 14 * * *', async () => {
    ...
    try {
        ...
        if (todaysOrders.length > 0) {
            ...
            console.log("2:01 PM: Grand Ledger sent and status reset to completed.");
        }
    } catch (err) {
